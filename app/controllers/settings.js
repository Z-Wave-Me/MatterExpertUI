...
appController.controller('SettingsAppController', function ($scope, $timeout, $window, $interval, $location, $q, $filter,cfg,dataService,deviceService) {
    ...
    $scope.loadSettings = function() {
        ...
        $scope.settings.input = cfg.mattercfg;
        $scope.settings.lastTZ = cfg.mattercfg.time_zone.replace(/_/g, ' ');
...
appController.controller('SettingsAppFormatController', function ($scope, $timeout, $window, $interval, $location, $q, cfg,dataService,deviceService) {
    $scope.settings = {
        input: cfg.mattercfg || {},
        lastTZ: cfg.mattercfg.time_zone,
        countdown: 60
    };
...
appController.controller('SettingsReportController', function ($scope, $window, $route, cfg,dataService,deviceService) {
    ...
    $scope.loadRemoteAccess = function () {
        dataService.getApi('endpoints', '/RemoteAccess').then(function (response) {
    ...
    $scope.sendReport = function (form, input) {
        ...
        if ($scope.ZMatterApiData) {
            input.matter_binding = 1;
            input.matter_vesion = $scope.ZMatterApiData.controller.data.softwareRevisionVersion.value;
            input.controller_info = JSON.stringify($scope.ZMatterApiData.controller.data);
        }
        if ($scope.builtInfo) {
