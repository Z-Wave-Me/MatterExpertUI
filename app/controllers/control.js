/**
 * @overview This is used to control the Matter controller itself and ot manage the Matter network.
 * @author Martin Vach
 */
...
appController.controller('ControlController', function ($scope, $interval, $timeout, $filter, $window, cfg, dataService, deviceService) {
    ...
    $scope.loadMatterData = function () {
        dataService.loadZMatterApiData().then(function (ZMatterAPIData) {
            setControllerData(ZMatterAPIData);
            setDeviceData(ZMatterAPIData);
     ...
     */
    function setControllerData(ZMatterAPIData) {
        ...
        var productId = $filter('hasNode')(ZMatterAPIData,'devices.' + nodeId + '.data.radioBoardName.value');
        var appVersionMajor = $filter('hasNode')(ZMatterAPIData,'devices.' + nodeId + '.data.EmberZNetVersionMajor.value');
        var appVersionMinor = $filter('hasNode')(ZMatterAPIData,'devices.' + nodeId + '.data.EmberZNetVersionMinor.value');
        */
     ...
     * @param {object} ZMatterAPIData
     */
    function setDeviceData(ZMatterAPIData) {
        angular.forEach(ZMatterAPIData.devices, function (node, nodeId) {
            if (nodeId == ZMatterAPIData.controller.data.nodeId.value) {
                return;
            }
            // Devices
...
/**
 * Shall inclusion be done using external BLE
 * @class SetLongRangeInclusionController
 *
 */
appController.controller('SetBLEWSExtController', function ($scope) {
    /**
...
/**
 * This turns the Matter controller into an inclusion/exclusion mode that allows including/excluding a device.
...
appController.controller('IncludeExcludeDeviceController', function ($scope, $route) {
    ...
    $scope.addNodeToNetwork = function (flag) {
        ...
        $scope.runMatterCmd('controller.AddNodeToNetwork(' + flag + ')');
        $route.reload();
    };
...
/**
 * It will change Matter controller own Home ID to the Home ID of the new network
...
appController.controller('IncludeDifferentNetworkController', function ($scope, $timeout, $window, cfg, dataService) {
    ...
    $scope.runIncludeToNetwork = function (cmd) {
        ...
        dataService.runMatterCmd(cfg.store_url + cmd).then(function (response) {
            ...
            $scope.controlDh.includeToNetworkTimeout = $timeout(function () {
                dataService.runMatterCmd(cfg.store_url + 'controller.SetLearnMode(0)');
                $scope.controlDh.network.modal = true;
            }, timeout);
        }, function (error) {
    ...
    $scope.requestNetworkUpdate = function (cmd, message, id) {
        ...
        dataService.runMatterCmd(cfg.store_url + cmd).then(function () {
            $timeout(function () {
...
/**
 * Restore Matter controller from the backup
...
/**
 * This controller will perform a soft restart and a reset of the Matter controller chip.
 * @class MatterChipRebootResetController
 *
 */
appController.controller('MatterChipRebootResetController', function ($scope, cfg, dataService) {
    /**
     * This function will perform a soft restart of the  firmware of the Matter controller chip
    ...
    /**
     * This function erases all values stored in the Matter chip and sent the chip back to factory defaults.
...
appController.controller('RequestNifAllController', function ($scope, $timeout, $window, cfg, dataService, deviceService) {
    ...
    $scope.requestNifAll = function (spin, $event) {
        ...
        dataService.runMatterCmd(cfg.call_all_nif).then(function (response) {
...
appController.controller('SetPromiscuousModeController', function ($scope) {
    ...
    $scope.setPromiscuousMode = function (cmd) {
        $scope.runMatterCmd(cmd, 1000, true);
    };
});
