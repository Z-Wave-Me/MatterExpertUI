...
//            nodeOverlap: 20,
     ...
     * Load matter API
     */
    $scope.loadZMatterApi = function () {
        dataService.loadZMatterApiData().then(function (ZMatterAPIData) {
            $scope.networkmap.cytoscape.elements = setCyElements(ZMatterAPIData);
    ...
    $scope.loadZMatterApi();

    /// --- Private functions --- ///
    /**
     * Set cytoscape elements
     * @param {type} ZMatterAPIData
     * @returns {obj}
     */
    function setCyElements(ZMatterAPIData) {
        var edges = {};
        var obj = {
            nodes: [],
            edges: []
        };
        var ctrlNodeId = ZMatterAPIData.controller.data.nodeId.value;
