...
appController.controller('TypeController', function($scope, $filter, $timeout,$interval,$q,dataService, cfg,_,deviceService) {
    ...
    $scope.allSettled = function () {
        ...
        $q.allSettled(promises).then(function (response) {
            ...
            // matterData error message
            if (matterData.state === 'rejected') {
            ...
            // Success - matterData
            if (matterData.state === 'fulfilled') {
                setData(matterData.value);
    ...
    function setData(ZMatterAPIData) {
        ...
        angular.forEach(ZMatterAPIData.devices, function(node, nodeId) {
            ...
            if (parseInt(nodeId, 10) === cfg.controller.zmatterNodeId) {
            ...
            var vendorName = isController? ZMatterAPIData.controller.data.vendor.value : node.data.vendorString.value;
            var zddXmlFile = $filter('hasNode')(node, 'data.ZDDXMLFile.value');
            //var productName = null;
            var fromSdk = true;
            var sdk;
            // SDK
            if ((node.data.SDK && node.data.SDK.value!== '') || (isController && ZMatterAPIData.controller.data.SDK && ZMatterAPIData.controller.data.SDK.value !== '')) {
                sdk = isController? ZMatterAPIData.controller.data.SDK.value : node.data.SDK.value;
            } else {
                //sdk = major + '.' + minor;
            ...
            // Matter plus
            var hasMatterPlusInfoCc = deviceService.hasCommandClass(node,94);
            var MatterPlusInfo = false;
            ...
            obj['mwief'] = mwief;
           // obj['ddr'] = ddr;
            obj['MatterPlusInfo'] = MatterPlusInfo;
            ...
            if(findIndex > -1){
                ...
                if (zddXmlFile) {
                    dataService.xmlToJson(cfg.server_url + cfg.zddx_url + zddXmlFile).then(function (response) {
                        $scope.productNames[nodeId] = response.MatterDevice.deviceDescription.productName;
                    });
                }
