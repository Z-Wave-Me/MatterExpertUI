...
appController.controller('NotificationController', function ($scope, $filter, $timeout, $interval, dataService, cfg, deviceService, $cookies, $route, _) {
    ...
    $scope.loadXmlData = function () {
        dataService.xmlToJson(cfg.server_url + cfg.alarms_url).then(function (response) {
            $scope.loadMatterData(response.Alarms.Alarm);
        }, function (error) {
     ...
     * Load matter data
     */
    $scope.loadMatterData = function (alarms) {
        dataService.loadZMatterApiData().then(function (ZMatterAPIData) {
            setData(ZMatterAPIData, alarms);
            ...
            $scope.refreshMatterData(alarms);
        }, function (error) {
            alertify.alertError($scope._t('error_load_data'));
        });
    };
     ...
     * @param {object}  alarms
     */
    $scope.refreshMatterData = function (alarms) {
     ...
     * @param alarms
     */
    function setData(ZMatterAPIData, alarms) {
        $scope.controllerId = ZMatterAPIData.controller.data.nodeId.value;
        // console.log(ZMatterAPIData, alarms);
