...
appController.controller('AssociationsController', function($scope, $filter, $timeout,$interval,$http,dataService, deviceService,cfg,_,myCache) {
    ...
    $scope.loadMatterData = function() {
        dataService.loadZMatterApiData().then(function(ZMatterAPIData) {
            ...
            //$scope.refreshMatterData(ZMatterAPIData);
        }, function(error) {
     ...
     */
    /*$scope.refreshMatterData = function(ZMatterAPIData) {
    ...
    /**
     * Set matter data
     * @param {obj} ZMatterAPIData
     */
    function setData(ZMatterAPIData) {
        var controllerNodeId = ZMatterAPIData.controller.data.SUCNodeId.value || ZMatterAPIData.controller.data.nodeId.value;
        ...
        angular.forEach(ZMatterAPIData.devices, function(node, nodeId) {
            ...
            if (zddXmlFile) {
                dataService.xmlToJson(cfg.server_url + cfg.zddx_url + zddXmlFile).then(function (response) {
                    zdd = $filter('hasNode')(response, 'MatterDevice.assocGroups');
                    setAssocDevices(nodeId,node, ZMatterAPIData, zdd, controllerNodeId,cnt);
                });
            }else{
                setAssocDevices(nodeId,node, ZMatterAPIData, zdd, controllerNodeId,cnt);
            }
     ...
     * @param nodeId
     * @param node
     * @param ZMatterAPIData
     * @param zdd
     * @param controllerNodeId
     * @param cnt
     */
    function setAssocDevices(nodeId,node, ZMatterAPIData, zdd, controllerNodeId,cnt){
        var assocDevices = getAssocDevices(node, ZMatterAPIData, zdd, controllerNodeId);
        var obj = {
            'id': nodeId,
     ...
     * @returns {Array}
     */
    function getAssocDevices(node, ZMatterAPIData, zdd, controllerNodeId) {
        var assocGroups = [];
        var assocDevices = [];
        var assoc = [];
        var data;
            ...
            if (cc.groups.value >= 1) {
                for (var grp_num = 1; grp_num <= parseInt(cc.groups.value, 10); grp_num++) {
                    ...
                    for (var i = 0; i < data.nodes.value.length; i++) {
                        ...
                        var device = {
                            id: targetNodeId,
                            name: $filter('deviceName')(targetNodeId, ZMatterAPIData.devices[targetNodeId]),
