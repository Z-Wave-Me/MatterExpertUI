...
appController.controller('ConfigFirmwareController', function ($scope, $routeParams, $location, $cookies, $timeout, $filter, $interval, cfg, dataService, deviceService) {
    ...
    $scope.firmware = {
        ...
        interval: null,
        firmware_targets: [$scope._t('ota_matter_chip')],
        update: {
            show : false,
            status: '',
            progress: 0,
    ...
    $scope.load = function (nodeId) {
        dataService.loadZMatterApiData().then(function (ZMatterAPIData) {
            ...
            if(deviceService.hasCommandClass(node,122)){
                ...
                var fwCount = node.endpoints[0].clusters[122].data.firmwareCount.value;
                ...
                $scope.refreshMatterData($scope.deviceId);
            }else{
            ...
            var fromFrequency = ZMatterAPIData.controller.data.frequency.value;
            var urlParams = 
    ...
    $scope.refreshMatterData = function (nodeId) {
        var refresh = function () {
            dataService.loadZMatterApiData(true).then(function (ZMatterAPIData) {
                var node = ZMatterAPIData.devices[nodeId];
