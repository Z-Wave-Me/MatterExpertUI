...
appController.controller('ConfigAssocController', function($scope, $filter, $routeParams, $location, $cookies, $timeout, $window,$interval, dataService, deviceService, myCache, cfg, _) {
    ...
    // Load data
    $scope.loadMatterData = function(nodeId, noCache) {
        dataService.loadZMatterApiData().then(function(ZMatterAPIData) {
            ...
            dataService.getCfgXml().then(function (cfgXml) {
                $scope.cfgXml = cfgXml;
                setData(node, ZMatterAPIData, nodeId, cfgXml);
            }, function(error) {
                setData(node, ZMatterAPIData, nodeId, {});
            });
        ...
        angular.forEach(node.endpoints, function(endpoint, index) {
            ...
            if (0x85 in endpoint.clusters) {
                for (var group = 0; group < endpoint.clusters[0x85].data.groups.value; group++) {
                   dataService.runMatterCmd(cfg.store_url + 'devices[' + nodeId + '].endpoints[' + index + '].clusters[0x85].Get(' + (group + 1) + ')', false, $scope._t('error_handling_data'), true);
                }
            }
            if (0x8e in endpoint.clusters) {
                for (var group = 0; group < endpoint.clusters[0x8e].data.groups.value; group++) {
                    dataService.runMatterCmd(cfg.store_url + 'devices[' + nodeId + '].endpoints[' + index + '].clusters[0x8e].Get(' + (group + 1) + ')', false, $scope._t('error_handling_data'), true);
    ...
    $scope.handleAssocModal = function(modal,$event,group) {
        ...
        console.log($scope.ZMatterAPIData.devices);
        angular.forEach($scope.ZMatterAPIData.devices, function(node, nodeId) {
    ...
    $scope.closeAssocModal = function() {
        ...
        $timeout(function() {
            $scope.toggleRowSpinner();
            $scope.loadMatterData($scope.nodeCfg.id);
        }, 3000);
     ...
     * Set matter data
     */
    function setData(node, ZMatterAPIData, nodeId, cfgXml) {
        var zddXmlFile = $filter('hasNode')(node, 'data.ZDDXMLFile.value');
        //  zddXmlFile not available
        if (!zddXmlFile || zddXmlFile === 'undefined') {
            $scope.assocGroups = getAssocGroups(node, null, nodeId, ZMatterAPIData, cfgXml);
        ...
        dataService.xmlToJson(cfg.server_url + cfg.zddx_url + zddXmlFile).then(function (zddXmlData) {
            var zdd = $filter('hasNode')(zddXmlData, 'MatterDevice.assocGroups');
            $scope.assocGroups = getAssocGroups(node, zdd, nodeId, ZMatterAPIData, cfgXml);
            if ($scope.assocGroups.length < 1) {
                $scope.alert = {message: $scope._t('no_association_groups_found'), status: 'alert-warning', icon: 'fa-exclamation-circle'};
            }
        });
     ...
     * Get assoc groups
     */
    function getAssocGroups(node, zdd, nodeId, ZMatterAPIData, cfgXml) {
        var assocGroups = [];
        var groupZdd = [];
        if (zdd) {
                ...
                for (var group = 0; group < groups; group++) {
                    ...
                    if ((0x85 in endpoint.clusters) && (group < endpoint.clusters[0x85].data.groups.value)) {
                        ...
                        for (var i = 0; i < $filter('unique')(groupDevices).length; i++) {
                            ...
                            var targetNode = ZMatterAPIData.devices[targetNodeId];
