...
appController.controller('ConfigHealthController', function ($scope, $routeParams, $timeout, $location, $cookies, $filter, $interval, cfg, deviceService, dataService) {
  ...
  $scope.load = function () {
    ...
    dataService.loadZMatterApiData().then(function (ZMatterAPIData) {
      ...
      $scope.health.ctrlNodeId = ZMatterAPIData.controller.data.nodeId.value;
      var node = ZMatterAPIData.devices[$routeParams.nodeId];
      /*if (!node || deviceService.notDevice(ZMatterAPIData, node, $routeParams.nodeId)) {
          return;
      }*/
      ...
      setDevice(node);
      setData(ZMatterAPIData, neighbours);
      $scope.refreshData();
  ...
  // Run Matter NOP Command
  $scope.runMatterNopCmd = function (cmd) {
    for (i = 0; i < 21; i++) {
  ...
  function setDevice(node) {
    angular.forEach(node.endpoints, function (endpoint, endpointId) {
      if (endpoint.clusters[115]) {
        $scope.health.device.hasPowerLevel = endpoint.clusters[115].data;
        $scope.health.cmd.testNodeInstance = endpointId;
      }
    });
  }
   ...
   * @returns {undefined}
   */
  function setData(ZMatterAPIData, neighbours) {
    angular.forEach(ZMatterAPIData.devices, function (node, nodeId) {
      ...
      var obj = {
        ...
        indicator: indicator,
        cmdTestNode: 'devices[' + $routeParams.nodeId + '].endpoints[' + $scope.health.cmd.testNodeInstance + '].clusters[115].TestNodeSet(' + nodeId + ',6,20)',
        cmdNop: 'devices[' + $routeParams.nodeId + '].endpoints[' + $scope.health.cmd.testNodeInstance + '].clusters[32].Get()'
      };
