...
appController.controller('BaseController', function ($scope, $rootScope, $cookies, $filter, $location, $anchorScroll, $window, $route, $interval, $timeout, cfg, dataService, deviceService, myCache) {
   ...
   * @param {int} timeout
   */
  $scope.runMatterCmd = function (cmd, timeout, hideError) {
    timeout = timeout || 1000;
    $scope.toggleRowSpinner(cmd);
    dataService.runMatterCmd(cfg.store_url + cmd).then(function (response) {}, function (error) {
      if (!hideError) {
   ...
   * @returns {undefined}
   */
  $scope.loadMatterConfig = function (nocache) {
    // Set config
  };
  ...
  $scope.setDongle = function () {
    dataService.getApi('matter_list').then(function (response) {
  ...
  $scope.setTimeStamp = function () {
    dataService.getApi('time', null, true).then(function (response) {
      ...
      cfg.mattercfg.time_zone = response.data.data.localTimeZone;
  ...
  $scope.loadBoxApiData = function () {
    dataService.loadZMatterApiData().then(function (ZMatterAPIData) {
      ...
      //var homeId = ZMatterAPIData.controller.data.homeId.value;
      var zmatterNodeId = ZMatterAPIData.controller.data.nodeId.value;
      //var APIVersion = ZMatterAPIData.controller.data.APIVersion.value;
      //var showAnalytics = true;
      // Changes MK
      //$scope.boxData.controller.controllerState = ZMatterAPIData.controller.data.controllerState.value;
      // Rewrite config
      var cfgController = {
        //homeName: ZMatterAPIData.controller.data.homeName.value || cfg.controller.homeName,
        //homeId: homeId,
        //homeIdHex: '0x' + ('00000000' + (homeId + (homeId < 0 ? 0x100000000 : 0)).toString(16)).slice(-8),
        hasDevices: hasDevices < 2 ? false : true,
        zmatterNodeId: zmatterNodeId,
