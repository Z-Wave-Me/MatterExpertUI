...
appController.controller('RouteMapController', function ($scope, $q,$interval, $filter, $window,$timeout,cfg, dataService,deviceService, myCache, _) {
    ...
    $scope.allSettled = function () {
        ...
        $q.allSettled(promises).then(function (response) {
            ...
            if (ZMatterAPIData.state === 'rejected' || packetApi.state === 'rejected') {
                alertify.alertError($scope._t('error_load_data'));
                return;
            }

            // Success - matterData
            if (ZMatterAPIData.state === 'fulfilled' && packetApi.state === 'fulfilled') {
                //console.log(cfg.mattercfg.node_positions)
                // Is routemap image in the matter config?
                if(cfg.mattercfg.routemap_img){
                    $scope.routeMap.bcgImage = cfg.server_url + cfg.load_image + cfg.mattercfg.routemap_img;
                }/*else{
                    $scope.routeMap.bcgImage = cfg.server_url + cfg.load_image + 'floorplan.jpg';
                }*/
                // Set positions
                var positions = cfg.mattercfg.node_positions;
                var packets = packetApi.value.data.data;
                if (_.size(packets)) {
                    zrp = new MatterRoutesPlotLib(new MatterNetworkAnalyticsLib(ZMatterAPIData.value, packets, positions));
    ...
    $scope.uploadFloorImage = function (files,info,spinner) {
        ...
        dataService.uploadApiFile(cmd, fd).then(function (response) {
            ...
            $scope.updateMatterConfig({routemap_img: files[0].name});
            $timeout(function () {
             alertify.dismissAll();
             $window.location.reload();
             }, 2000);
    ...
    $scope.saveNodesPositions = function(nodes){
        ...
        $scope.updateMatterConfig(input);
    }
     ...
     * @param {object} nodes
     */
    $scope.updateMatterConfig = function(input){
    ...
    $scope.manualRouteSet = function(route, deleted) {
        ...
        dataService.runMatterCmd(cfg.store_url + 'devices[' + src + '].AssignPriorityReturnRoute(' + dst + ', ' + route.join(',') + ')').then(function (response) {
            if (deleted) {
