...
appController.controller('ZnifferController', function ($scope, $interval, $timeout, $cookies, $location, $http, $filter, cfg, dataService, deviceService, myCache, _) {
    ...
    $scope.loadMatterData = function() {
        dataService.loadZMatterApiData(true).then(function(ZMatterAPIData) {
            var appVersion = parseFloat(ZMatterAPIData.controller.data.APIVersion.value);
            $scope.zniffer.promiscModeAvailable = appVersion >= 5.39; // Promisc appeared in this version
            $scope.zniffer.promiscModeEnabled = ZMatterAPIData.controller.data.promiscMode.value;
        });
    };
