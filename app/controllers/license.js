...
appController.controller('LicenseController', function($scope, $timeout, dataService) {
    ...
    $scope.loadMatterData = function() {
        dataService.loadZMatterApiData().then(function(ZMatterAPIData) {
            //$scope.controllerUuid = ZMatterAPIData.controller.data.uuid.value;
            var appVersion = ZMatterAPIData.controller.data.APIVersion.value.split('.');
            $scope.controllerIsZeroUuid = parseInt("0x" + ZMatterAPIData.controller.data.uuid.value, 16) === 0;
            // Form data
            $scope.formData.uuid = ZMatterAPIData.controller.data.uuid.value;
