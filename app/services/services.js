...
appService.service('deviceService', function ($filter, $log, $cookies, $window, $location, cfg, _) {
    ...
    this.setDongle = function (dongle) {
        angular.extend(cfg, {
            'update_url': '/ZMatter.' + dongle + '/Data/',
            'store_url': '/ZMatter.' + dongle + '/Run/',
            'restore_url': '/ZMatter.' + dongle + '/Restore',
            'queue_url': '/ZMatter.' + dongle + '/InspectQueue',
            'fw_update_url': '/ZMatter.' + dongle + '/FirmwareUpdate',
            'zme_bootloader_upgrade': '/ZMatter.' + dongle + '/ZMEBootloaderUpgrade',
            'zme_firmware_upgrade': '/ZMatter.' + dongle + '/ZMEFirmwareUpgrade',
            'upload_bootloader': '/ZMatter.' + dongle + '/ZMEBootloaderUpgrade',
            'upload_firmware': '/ZMatter.' + dongle + '/ZMEFirmwareUpgrade',
            'license_load_url': '/ZMatter.' + dongle + '/ZMELicense',
            'stat_url': '/ZMatter.' + dongle + '/CommunicationStatistics',
            'postfixget_url': '/ZMatter.' + dongle + '/PostfixGet',
            'postfixadd_url': '/ZMatter.' + dongle + '/PostfixAdd',
            'postfixremove_url': '/ZMatter.' + dongle + '/PostfixRemove',
            'checklinks': '/ZMatter.' + dongle + '/CheckAllLinks',
            'zniffer_url': '/ZMatter.' + dongle + '/Zniffer',
            'communication_history_url': '/ZMatter.' + dongle + '/CommunicationHistory',
            'rssi_chart': '/ZMatter.' + dongle + '/RSSIGet',
            'call_all_nif': '/ZMatter.' + dongle + '/CallForAllNIF',
            'test_node': '/ZMatter.' + dongle + '/TestNode/',
            'network_statistics': '/ZMatter.' + dongle + '/Run/',
            'post_report_api': '/ZMatter.' + dongle + '/sendZWayReport',
            'reorg_run_url': '/ZMatter.' + dongle + '/NetworkReorganization',
            'reorg_log_url': '/ZMatter.' + dongle + '/GetReorganizationLog',
            'zddx_create_url': '/ZMatter.' + dongle + '/CreateZDDX/',
            'get_network_statistics': '/ZMatter.' + dongle + '/GetStatisticsData',
            'clear_network_statistics': '/ZMatter.' + dongle + '/ClearStatisticsData',
     ...
     */
    this.notDevice = function (ZMatterAPIData, node, nodeId) {
        return false;
    };
    ...
    this.hasCommandClass = function (node, ccId, instId) {
        var hasCc = false;
        if (instId) {
            hasCc = node.endpoints[instId] && node.endpoints[instId].clusters[ccId];
        } else {
            angular.forEach(node.endpoints, function (endpoint, endpointId) {
                if (endpoint.clusters[ccId]) {
                    hasCc = endpoint.clusters[ccId];
                    return;
                }
            });
        }
     ...
     */
    this.allInterviewsDone = function (endpoints) {
        var interviewDone = true;
        for (var iId in endpoints) {
            for (var ccId in endpoints[iId].clusters) {
                var isDone = endpoints[iId].clusters[ccId].data.interviewDone.value;
                if (isDone === false) {
                    return false
                }
            }
    ...
    */
     ...
     */
    this.configGetNav = function (ZMatterAPIData) {
        return configGetNav(ZMatterAPIData);
    };

    this.getNavConfig = function (devices, controllerNodeId) {
        return Object.entries(devices).filter(([nodeId, node]) => true)
     ...
     *  Get expert commands
     */
    this.configGetCommands = function (methods, ZMatterAPIData) {
        return configGetCommands(methods, ZMatterAPIData);
    };
     ...
     *  Get interview stage
     */
    this.configInterviewStage = function (ZMatterAPIData, id, languages) {
        return configInterviewStage(ZMatterAPIData, id, languages);
    };
     ...
     *  Switch all cont
     */
    this.configSwitchAllCont = function (node, nodeId, ZMatterAPIData, cfgXml) {
        return configSwitchAllCont(node, nodeId, ZMatterAPIData, cfgXml);
    };

    /**
     * Protection cont
     */
    this.configProtectionCont = function (node, nodeId, ZMatterAPIData, cfgXml) {
        return configProtectionCont(node, nodeId, ZMatterAPIData, cfgXml);
    };

    /**
     * Wakeup cont
     */
    this.configWakeupCont = function (node, nodeId, ZMatterAPIData, cfgXml) {
        return configWakeupCont(node, nodeId, ZMatterAPIData, cfgXml);
    };
     ...
     */
    this.buildCfgXml = function (data, cfgXml, id, cluster) {
        return buildCfgXml(data, cfgXml, id, cluster);
    };
    ...
    function isLocalyReset(node) {
        ...
        for (var iId in node.endpoints) {
            if (node.endpoints[iId].clusters[90] && node.endpoints[iId].clusters[90].data.reset.value) {
                isLocalyReset = true;
            }
        }
     ...
     *  Get config navigation devices
     */
    function configGetNav(ZMatterAPIData) {
        var devices = [];
     ...
     *  Get expert commands
     */
    function configGetCommands(methods, ZMatterAPIData) {
        var methodsArr = [];
        angular.forEach(methods, function (params, method) {
            //str.split(',');
            var cmd = {};
            var defaultValues = method_defaultValues(ZMatterAPIData, methods[method]);
            cmd['data'] = {
                'method': method,
    ...
    function configGetInterviewCommands(node, updateTime) {
        ...
        for (var iId in node.endpoints) {
            ...
            for (var ccId in node.endpoints[iId].clusters) {
                ...
                obj['iId'] = iId;
                obj['ccId'] = ccId;
                obj['ccName'] = node.endpoints[iId].clusters[ccId].name;
                obj['interviewDone'] = node.endpoints[iId].clusters[ccId].data.interviewDone.value;
                obj['cmdData'] = node.endpoints[iId].clusters[ccId].data;
                obj['cmdDataIn'] = node.endpoints[iId].data;
     ...
     *  Get interview stage
     */
    function configInterviewStage(ZMatterAPIData, id, languages) {
        var istages = [];
        istages.push((ZMatterAPIData.devices[id].data.nodeInfoFrame.value && ZMatterAPIData.devices[id].data.nodeInfoFrame.value.length) ? '+' : '-');
        istages.push('&nbsp;');
        istages.push((0x86 in ZMatterAPIData.devices[id].endpoints[0].clusters) ? (ZMatterAPIData.devices[id].endpoints[0].clusters[0x86].data.interviewDone.value ? '+' : (ZMatterAPIData.devices[id].endpoints[0].clusters[0x86].data.interviewCounter.value > 0 ? '.' : '&oslash;')) : '+'); // Version
        istages.push((0x72 in ZMatterAPIData.devices[id].endpoints[0].clusters) ? (ZMatterAPIData.devices[id].endpoints[0].clusters[0x72].data.interviewDone.value ? '+' : (ZMatterAPIData.devices[id].endpoints[0].clusters[0x72].data.interviewCounter.value > 0 ? '.' : '&oslash;')) : '+'); // ManufacturerSpecific
        istages.push((0x60 in ZMatterAPIData.devices[id].endpoints[0].clusters) ? (ZMatterAPIData.devices[id].endpoints[0].clusters[0x60].data.interviewDone.value ? '+' : (ZMatterAPIData.devices[id].endpoints[0].clusters[0x60].data.interviewCounter.value > 0 ? '.' : '&oslash;')) : '+'); // MultiChannel
        var moreCCs = false;
        for (var i in ZMatterAPIData.devices[id].endpoints) {
            istages.push('&nbsp;');
            var endpoint = ZMatterAPIData.devices[id].endpoints[i];
            for (var cc in endpoint.clusters) {
                moreCCs = true;
                ...
                istages.push(endpoint.clusters[cc].data.interviewDone.value ? '+' : (endpoint.clusters[cc].data.interviewCounter.value > 0 ? '.' : '&oslash;'));
            }
        }
        ;
        if (!moreCCs) {
    ...
    function configConfigCont(node, nodeId, zddXml, cfgXml, lang, languages) {
        if (!0x70 in node.endpoints[0].clusters) {
            return null;
        }
        if (!zddXml) {
            return null;
        ...
        if (!zddXml.MatterDevice.hasOwnProperty("configParams")) {
            return null;
        }
        var config_cont = [];
        var params = zddXml.MatterDevice.configParams['configParam'];
        ...
        angular.forEach(params, function (conf_html, i) {
            ...
            // get value from the Matter data
            var config_matter_value = null;

            if (angular.isDefined(node.endpoints[0].clusters[0x70])) {
                if (node.endpoints[0].clusters[0x70].data[conf_num] != null && node.endpoints[0].clusters[0x70].data[conf_num].val.value !== "") {
                    config_matter_value = node.endpoints[0].clusters[0x70].data[conf_num].val.value;
                    conf_default = config_matter_value;
            ...
            if (cfgFile[conf_num] !== undefined) {
                ...
                if (config_matter_value !== null) {
                    config_config_value = config_matter_value;
                } else {
            ...
            var updateTime = '';
            if (angular.isDefined(node.endpoints[0].clusters[0x70])
                    && angular.isDefined(node.endpoints[0].clusters[0x70].data[conf_num])) {
                var uTime = node.endpoints[0].clusters[0x70].data[conf_num].updateTime;
                var iTime = node.endpoints[0].clusters[0x70].data[conf_num].invalidateTime;
            ...
            switch (conf_type) {
                ...
                case 'rangemapped':
                    ...
                    conf_method_descr = {
                        ...
                        configMatterValue: config_matter_value,
                        confNum: conf_num,
                        confSize: conf_size
                    };
                    break;
     ...
     * Switch all cont
     */
    function configSwitchAllCont(node, nodeId, ZMatterAPIData, cfgXml) {
        var switchall_cont = false;
        if (0x27 in node.endpoints[0].clusters) {
            var cfgFile = getCfgXmlParam(cfgXml, nodeId, '0', '27', 'Set');
            var uTime = node.endpoints[0].clusters[0x27].data.mode.updateTime;
            var iTime = node.endpoints[0].clusters[0x27].data.mode.invalidateTime;
            var updateTime = $filter('isTodayFromUnix')(uTime);
            var isUpdated = (uTime > iTime ? true : false);
            var gui_descr = getMethodSpec(ZMatterAPIData, nodeId, 0, 0x27, 'Set');
            ...
            switchall_cont = {
                ...
                confNum: 0,
                confSize: 0,
                cmd: 'devices[' + nodeId + '].endpoints[0].clusters[0x27]'
            };
     ...
     * Protection cont
     */
    function configProtectionCont(node, nodeId, ZMatterAPIData, cfgXml) {
        var protection_cont = false;
        if (0x75 in node.endpoints[0].clusters) {
            var cfgFile = getCfgXmlParam(cfgXml, nodeId, '0', '75', 'Set');
            var uTime = node.endpoints[0].clusters[0x75].data.state.updateTime;
            var iTime = node.endpoints[0].clusters[0x75].data.state.invalidateTime;
            var updateTime = $filter('isTodayFromUnix')(uTime);
            var isUpdated = (uTime > iTime ? true : false);
            var gui_descr = getMethodSpec(ZMatterAPIData, nodeId, 0, 0x75, 'Set');
            var conf_default_value = 0;
            ...
            protection_cont = {
                ...
                confNum: 0,
                confSize: 0,
                cmd: 'devices[' + nodeId + '].endpoints[0].clusters[0x75]'
            };
        }
        ;
     ...
     * Wakeup cont
     */
    function configWakeupCont(node, nodeId, ZMatterAPIData, cfgXml) {
        var wakeup_cont = false;
        if (0x84 in node.endpoints[0].clusters) {
            var cfgFile = getCfgXmlParam(cfgXml, nodeId, '0', '84', 'Set');
            var wakeup_matter_min = (node.endpoints[0].clusters[0x84].data.version.value == 1) ? 0 : node.endpoints[0].clusters[0x84].data.min.value;
            var wakeup_matter_max = (node.endpoints[0].clusters[0x84].data.version.value == 1) ? 0xFFFFFF : node.endpoints[0].clusters[0x84].data.max.value;
            var wakeup_matter_value = node.endpoints[0].clusters[0x84].data.interval.value;
            var wakeup_matter_default_value = (node.endpoints[0].clusters[0x84].data.version.value == 1) ? 86400 : node.endpoints[0].clusters[0x84].data['default'].value; // default is a special keyword in JavaScript
            var wakeup_matter_nodeId = node.endpoints[0].clusters[0x84].data.nodeId.value;
            var uTime = node.endpoints[0].clusters[0x84].data.updateTime;
            var iTime = node.endpoints[0].clusters[0x84].data.invalidateTime;
            var updateTime = $filter('isTodayFromUnix')(uTime);
            var isUpdated = (uTime > iTime ? true : false);
            if (wakeup_matter_min !== '' && wakeup_matter_max !== '') {
                var gui_descr = getMethodSpec(ZMatterAPIData, nodeId, 0, 0x84, 'Set');
                gui_descr[0].type.range.min = parseInt(wakeup_matter_min, 10);
                gui_descr[0].type.range.max = parseInt(wakeup_matter_max, 10);
                ...
                if (angular.isArray(cfgFile) && cfgFile.length > 0) {
                    ...
                    if (wakeup_matter_value != "" && wakeup_matter_value != 0 && wakeup_matter_nodeId != "") {
                        // not defined in config: adopt devices values
                        wakeup_conf_value = parseInt(wakeup_matter_value, 10);
                    } else {
                        // values in device are missing. Use defaults
                        wakeup_conf_value = parseInt(wakeup_matter_default_value, 10);
                    }
                    ;
                }
                ;
                wakeup_cont = {
                    ...
                    updateTime: updateTime,
                    isUpdated: isUpdated,
                    defaultValue: wakeup_matter_default_value,
                    showDefaultValue: wakeup_matter_default_value,
                    configCconfigValue: wakeup_conf_value,
                    configCconfigNodeValue: wakeup_conf_node_value,
                    confNum: 0,
                    confSize: 0,
                    cmd: 'devices[' + nodeId + '].endpoints[0].clusters[0x84]'
                };
            } else {
    ...
    function isInCfgXml(data, cfgXml) {
        ...
        if (angular.isObject(cfgXml) && $filter('hasNode')(cfgXml, 'config.devices.deviceconfiguration')) {
            ...
            if (!_.isArray(hasCfgXml)) {
                ...
                obj['endpoint'] = hasCfgXml['_endpoint'];
                obj['cluster'] = hasCfgXml['_cluster'];
            ...
            angular.forEach(hasCfgXml, function (v, k) {
                ...
                var obj = {};
                obj['id'] = v['_id'];
                obj['endpoint'] = v['_endpoint'];
                obj['cluster'] = v['_cluster'];
     ...
     *Build config XML file
     */
    function buildCfgXml(data, cfgXml, id, cluster) {
        var hasCfgXml = false;
        var assocCc = [133, 142];
        /* var formData = [];
         if (cluster == '84') {
        ...
        if (angular.isObject(cfgXml) && $filter('hasNode')(cfgXml, 'config.devices.deviceconfiguration')) {
            ...
            angular.forEach(hasCfgXml, function (v, k) {
                var obj = {};
                if (v['_id'] == id && v['_cluster'] == cluster) {
                    return;
                }
                obj['id'] = v['_id'];
                obj['endpoint'] = v['_endpoint'];
    ...
    function buildCfgXmlFile(xmlData) {
        ...
        angular.forEach(xmlData, function (v, k) {
            if (_.isNumber(parseInt(v.id, 10))) {
                xml += '<deviceconfiguration id="' + v.id + '" endpoint="' + v.endpoint + '" cluster="' + v.cluster + '" command="' + v.command + '" parameter="' + v.parameter + '"/>' + "\n";
            }
        });
        xml += '</devices></config>' + "\n";
        return xml;
    ...
    /**
     * Get Matter session @returns {unresolved}
     */
    function sessionApi() {
        return $http({
            method: "get",
