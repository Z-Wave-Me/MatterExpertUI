...
configurationCommandsModule.service('configurationCommandsService', ['dataHolderService', 'deviceService', '$interval', 'cfg', '$rootScope', function (dataHolderService, deviceService, $interval, cfg, $rootScope) {
  ...
  this.init = function (nodeId) {
    ...
    return self.getCommands(nodeId).then((commands) => {
      ...
      dataUpdate = $rootScope.$on('configuration-commands:matter-data:update', function (_, {ids, data}) {
  ...
  function renderAllMethodSpec(ZMatterAPIData) {
    _methods_specs_rendered = {};

    for (var devId in ZMatterAPIData.devices) {
      _methods_specs_rendered[devId] = {};
      for (var instId in ZMatterAPIData.devices[devId].endpoints) {
        _methods_specs_rendered[devId][instId] = {};
        for (var ccId in ZMatterAPIData.devices[devId].endpoints[instId].clusters) {
          _methods_specs_rendered[devId][instId][ccId] = renderMethodSpec(parseInt(ccId, 10), ZMatterAPIData.devices[devId].endpoints[instId].clusters[ccId].data);
        }
      }
    }
  }
  ...
  function devicesHtmlSelectFilter(ZMatterAPIData, span, dev, type) {
    ...
    switch (type) {
      ...
      case 'dstnode':
        // skip not controller
        return dev != ZMatterAPIData.controller.data.nodeId.value;
      ...
      case 'node':
        ...
        return ZMatterAPIData.devices[dev].data.isSleepy.value;
  ...
  function methodDefaultValues(ZMatterAPIData, method) {
    ...
    function methodDefaultValue(val) {
      ...
      if ('string' in val['type'])
        return "";
      if ('node' in val['type'])
        for (var dev in ZMatterAPIData.devices) {
          if (devicesHtmlSelectFilter(ZMatterAPIData, null, dev, 'node')) {
            continue;
          }
          return parseInt(dev);
        }
