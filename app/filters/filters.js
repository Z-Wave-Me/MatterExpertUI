    ...
    return function (timestamp, invalidateTime) {
        ...
        var obj = {
            ...
            time: $filter('getFormattedTime')(
                d,
                false,
                cfg.mattercfg.time_format, d
            ),
            invalidateTime: (di ?
            $filter('getFormattedDate')(di) + ' '
            + $filter('getFormattedTime')(di, false, cfg.mattercfg.time_format)
                : ''),
...
angApp.filter('getFormattedDate', function (cfg) {
    return function (d) {
        ...
        switch (cfg.mattercfg.date_format) {
            case 'dd-mm-yyyy':
                return day + '-' + mon + '-' + year;
            case 'yyyy-mm-dd':
                return year + '-' + mon + '-' + day;
...
angApp.filter('deviceName', function (cfg, deviceService) {
    return function (deviceId, node) {
        ...
        if (deviceId == cfg.controller.zmatterNodeId) {
...
angApp.filter('associationExists', function () {
    return function (fromNode, toNodeId) {
        var exists = false;
        $.each(fromNode.endpoints, function (index, endpoint) {
            ...
            if (0x85 in endpoint.clusters) {
                for (var group = 0; group < endpoint.clusters[0x85].data.groups.value; group++) {
                    var associations = endpoint.clusters[0x85].data[group + 1].nodes.value;
            ...
            if (0x8e in endpoint.clusters) {
                for (var group = 0; group < endpoint.clusters[0x8e].data.groups.value; group++) {
                    var associations = endpoint.clusters[0x8e].data[group + 1].nodesInstances.value;
...
angApp.filter('getRoutesCount', function ($filter) {
    return function (ZMatterAPIData, nodeId) {
        ...
        var getFarNeighbours = function (nodeId, exludeNodeIds, hops) {
            ...
            if (hops > 2) // Matter allows only 4 routers, but we are interested in only 2, since network becomes unstable if more that 2 routers are used in communications
                return [];

            var nodesList = [];
            angular.forEach(ZMatterAPIData.devices[nodeId].data.neighbours.value, function (nnodeId, index) {
                if (!(nnodeId in ZMatterAPIData.devices))
                ...
                if (!in_array(nnodeId, exludeNodeIds)) {
                    ...
                    if (!ZMatterAPIData.devices[nnodeId].data.isSleepy.value)
 ...
 * Set matterPlus icon
 */
angApp.filter('matterPlusIcon', function () {
    return function (input) {
        //var icon = 'fa fa-minus';
        var icon = '&nbsp';
        if (input === true) {
